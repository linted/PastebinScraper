...
def main():
    ...
    parser.add_argument(
        "-s", "--smtp-server", help="smtp server to talk to", required=True
    )
    ...
    while 1:
        ...
        time.sleep(
            10
        )  # sleep for a second no matter what. pastes come in slow most of the time
...
def parse_paste(paste_id, connection_info):
    ...
    if not paste:
        ...
        return (
            None
        )  # currently we just fail if we have a problem when trying to get a paste
...
def send_results(results, connection_info):
    current_message = email.format(
        title=results["title"], id=results["id"], body=results["body"]
    )
    ...
    while retry:
        try:
            ...
            setup_email(
                connection_info["send_email"], password, connection_info["server"]
            )
